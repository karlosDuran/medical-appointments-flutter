...
import 'prueba.dart'; // Importación de la pantalla de prueba

void main() {
  // Inicialización de Widgets/Firebase envuelta en una función asíncrona que es llamada inmediatamente
  WidgetsFlutterBinding.ensureInitialized();

  // Ejecuta runApp SÓLO después de que Firebase haya terminado de inicializarse.
  ...
  Future<void> _signIn() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        // NAVEGACIÓN EXITOSA: Ir a la PruebaPage (¡'const' ha sido eliminado!)
        if (mounted) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => PruebaPage()),
          );
        }
      } on FirebaseAuthException catch (e) {
        String message = "";
